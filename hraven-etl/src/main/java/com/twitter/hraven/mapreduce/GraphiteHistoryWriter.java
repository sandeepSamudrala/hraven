...
public class GraphiteHistoryWriter {
   ...
   * 
   * Passes the large multi record of which this record is a part of,
   * so that we can get other contextual attributes to use in the graphite metric
   * naming scheme
   * 
   * @param serviceKey
   * @param jobRecord
  ...
  public GraphiteHistoryWriter(String prefix, HravenService serviceKey, JobHistoryMultiRecord records) {
  ...
  public String getOutput() throws IOException {
    ...
    for (JobHistoryRecord jobRecord : records) {
      if (service == HravenService.JOB_HISTORY
        ...
        /*
         * Send metrics in the format {PREFIX}.{cluster}.{user}.{appId}.{subAppId} {value} {submit_time}
         * 
         * subAppId is formed differently for each framework
         * 
         * For pig, it is the alias names of the job
         * 
         * appId will be parsed with a bunch of known patterns (oozie launcher jobs, pig jobs, etc.)
         */
  ...
  private Framework getFramework(JobHistoryMultiRecord records) {
    Object rec = records.getValue(RecordCategory.CONF_META, new RecordDataKey(Constants.FRAMEWORK_COLUMN));
  ...
  /**
   * Util method to sanitize metrics for sending to graphite
   * E.g. remove periods ("."), etc.
   * @throws UnsupportedEncodingException 
   */
